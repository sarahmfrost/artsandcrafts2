...
def test(image_path):
    ...
    test_image = scipy.misc.imresize(scipy.misc.imread(image_path, mode='RGB').astype('float32'),(IMAGE_HEIGHT_INPUT, IMAGE_WIDTH_INPUT))
    test_image = np.expand_dims(test_image, 0)
    test_image = test_image/255.

    test_data = tf.placeholder(tf.float32, [None, IMAGE_HEIGHT_INPUT, IMAGE_WIDTH_INPUT, N_CHANNEL])
    
    output = SRNetwork(test_data)
